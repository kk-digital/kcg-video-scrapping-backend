...
def http_get_downloadding_ingress_videos(offset: int = 0, limit: int = 1):
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json()["data"]
        request_logger.error("Error in http_get_downloadding_ingress_videos: %s", response.text)
        return None
    except Exception as e:
        request_logger.error("Error in http_get_downloadding_ingress_videos: %s", e)
        return None
...
def http_get_extracting_query(offset: int = 0, limit: int = 10):
    ...
    try:
        ...
        if response.status_code == 200:
            return response.json()["data"]
        request_logger.error("Error in http_get_extracting_query: %s", response.text)
        return None
    except Exception as e:
        request_logger.error("Error in http_get_extracting_query: %s", e)
        return None
